...
public static class SourceEndpoints
{
    public static IEndpointRouteBuilder MapSourceEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        group.MapGet("", async (RadarDbContext db, CancellationToken ct) => Results.Ok(await db.Sources.AsNoTracking().OrderBy(x => x.Name).Select(x => new SourceListResponse(x.Id, x.Name, x.Locator, x.Enabled, x.Priority, x.CreatedAt, x.FetchAttempts.OrderByDescending(a => a.AttemptedAt).Select(a => new FetchResponse(a.AttemptedAt, a.Succeeded, a.EntryCount, a.InsertedCount, a.SkippedCount, a.FailureCategory, a.Message)).FirstOrDefault())).ToListAsync(ct)));
        group.MapPost("", async (CreateSourceRequest request, RadarDbContext db, CancellationToken ct) =>
        {
            ...
            db.Sources.Add(source); await db.SaveChangesAsync(ct); return Results.Created($"/api/sources/{source.Id}", new SourceListResponse(source.Id, source.Name, source.Locator, source.Enabled, source.Priority, source.CreatedAt, null));
        });
...
public sealed record SourceListResponse(Guid Id, string Name, string Locator, bool Enabled, int Priority, DateTimeOffset CreatedAt, FetchResponse? LastFetch);
