...
import { MergeStoryControl, SplitItemControl } from "../../../components/story-corrections";
...
function StoryContent({ story }: { story: StoryDetail }) {
  return <><Link href="/" className="text-sm text-cyan-400 hover:text-cyan-300">← Daily brief</Link><p className="mt-10 text-sm font-semibold uppercase tracking-[0.3em] text-cyan-400">Story detail</p><h1 className="mt-3 text-4xl font-bold tracking-tight">{story.title}</h1><p className="mt-4 max-w-2xl text-lg text-slate-300">{story.summary}</p><MergeStoryControl storyId={story.id}/><section className="mt-12"><h2 className="text-sm font-semibold uppercase tracking-widest text-slate-500">Source provenance</h2><div className="mt-4 grid gap-4">{story.sourceItems.map(item => <article key={item.id} className="rounded-xl border border-slate-800 bg-slate-900/60 p-6"><h3 className="text-lg font-semibold">{item.title}</h3><dl className="mt-5 grid gap-3 text-sm"><div><dt className="text-slate-500">Source</dt><dd>{item.source.name} <span className="text-slate-500">({item.source.locator})</span></dd></div><div><dt className="text-slate-500">Canonical locator</dt><dd><a className="break-all text-cyan-400 hover:text-cyan-300" href={item.canonicalLocator}>{item.canonicalLocator}</a></dd></div><div><dt className="text-slate-500">Observed</dt><dd>{new Date(item.observedAt).toLocaleString()}</dd></div><div><dt className="text-slate-500">Story membership</dt><dd>{item.membershipMethod} ({item.membershipMethodVersion}): {item.membershipReason}</dd></div></dl><SplitItemControl storyId={story.id} sourceItemId={item.id}/></article>)}</div></section>{story.corrections.length > 0 && <section className="mt-12"><h2 className="text-sm font-semibold uppercase tracking-widest text-slate-500">Manual corrections</h2><ul className="mt-4 grid gap-2 text-sm text-slate-300">{story.corrections.map(correction => <li key={`${correction.action}-${correction.createdAt}`} className="rounded-lg border border-slate-800 p-3">{correction.action}: {correction.reason}</li>)}</ul></section>}</>;
}
